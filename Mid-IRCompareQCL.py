...
f = open('MID-IRCombustionProducts - CU background.pckl', 'wb')
...
f = open('MID-IRCombustionProducts - CU fine.pckl', 'rb')
[absorbance_cu, T, P, y, PL_cu, nu_cu, molecules] = pickle.load(f)
f.close()

f = open('MID-IRCombustionProducts - CU 1 GHz.pckl', 'rb')
[absorbance_cu_res, _, _, _, _, nu_cu_res, _] = pickle.load(f)
f.close()

f = open('MID-IRCombustionProducts - CU background.pckl', 'rb')
[absorbance_cu_back, _, _, _, _, _, _] = pickle.load(f)
f.close()

# f = open('MID-IRCombustionProducts - Ron.pckl', 'rb')
# [absorbance_ron, _, _, _, _, PL_ron, nu_ron, _] = pickle.load(f)
# f.close()

# f = open('MID-IRCombustionProducts - Ron res.pckl', 'rb')
# [absorbance_ron_res, _, _, _, _, nu_ron_res, _] = pickle.load(f)
# f.close()
...
# transmission_ron = np.exp(-absorbance_ron)
# lambda_ron = 10000000 / nu_ron

# transmission_ron_res = np.exp(-absorbance_ron_res)
# lambda_ron_res = 10000000 / nu_ron_res

# plt.figure(figsize=(6, 4), dpi=200, facecolor='w', edgecolor='k')  
# plt.rcParams.update({'figure.autolayout':True,'lines.linewidth':0.8})

# for i in range(len(T)):  
#     plt.plot(lambda_ron, transmission_ron[i,:], color=colors[i], label=str(T[i]) + ' K')
#     plt.plot(lambda_ron_res, transmission_ron_res[i,:], '-.', color=colors[i])
